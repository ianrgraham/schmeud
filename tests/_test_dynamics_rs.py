...
def test_d2min():
    ...
    b0 = np.array([[2, 1], [3, 4], [4, 5]])

    b = np.array([[6, 1], [6, 0], [4, 5]])

    ans1 = d2min_py(b0, b)
    sol = np.array([[3.74193548, 0.64516129], [-1.83870968, 0.09677419]])
...
def test_d2min_frame():
    ...
    for i in range(0, 40, 5):
        ...
        nlist = nlist_query.query(snap.particles.position, {
            'num_neighbors': 20,
            "exclude_ii": True
        }).toNeighborList()

        now = time.time()
        d2min = dynamics.d2min_frame(snap.particles.position[:, :2],
                                     snap_later.particles.position[:, :2],
                                     nlist.query_point_indices,
                                     nlist.point_indices, (sbox, sbox_later))
        print(time.time() - now)
